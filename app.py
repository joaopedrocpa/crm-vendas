...
def limpar_valor_monetario(valor):
    """
    Função Blindada: Lê tanto formato BR (1.000,00) quanto US (1000.00)
    """
    if pd.isna(valor): return 0.0
    s = str(valor).strip().replace('R$', '').strip()
    
    # Se tiver vírgula, assumimos que é decimal BR ou separador US
    # Prioridade para lógica BR: Se tem ',' assume que é decimal
    if ',' in s:
...
# --- SALVAMENTO (A CORREÇÃO ESTÁ AQUI) ---
def salvar_interacao_nuvem(cnpj, data_obj, tipo, resumo, vendedor, valor=0.0):
    try:
        ...
        # CORREÇÃO: Força formato Brasileiro (Vírgula) para o Google Sheets não confundir
...
def salvar_novo_lead_completo(cnpj, nome, contato, telefone, vendedor, origem, primeira_acao, resumo_inicial, valor_inicial=0.0):
    try:
        ...
        # CORREÇÃO: Força formato Brasileiro (Vírgula)
...
try:
    ...
    if df is not None and not df.empty:
        st.sidebar.title("🚀 CRM Master 7.5")
                ...
                if not filtro.empty and 'Data_Obj' in filtro.columns:
                    ...
                    try:
                        if pd.notna(ultima['Data_Obj']):
                            ...
                            if ultima['Tipo'] == 'WhatsApp Enviado
